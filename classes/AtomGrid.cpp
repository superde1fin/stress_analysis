...
vector<Atom> AtomGrid::find_neighbors(Atom* atm, set<int> exclude, map<int, map<int, float>> cutoffs){
...
//                            cout << atm_ptr -> get_id() << " " << dist << " " << atm_ptr -> get_type() <<  endl;
                            ...
                            if(dist < cutoffs[atm_ptr -> get_type()][atm -> get_type()] && !Helper::element_in(atm_ptr -> get_id(), exclude)){
                                if((atm -> get_type() == 3 || atm -> get_type() == 4) && dist < min_modifyier_dist){
                                    min_modifyier_dist = dist;
                                    neighbors.clear();
                                    }
//                                cout << "Added\n";
                                neighbors.push_back(*atm_ptr);
                                }
                            }
                        }
                    }
...
void AtomGrid::reset_grid(vector<Atom>* atoms_ptr){
    array<int, 3> key;
    array<float, 3> atom_position;
    for(int i = 0; i < AtomGrid::int_sides[0]; i++){
        for(int j = 0; j < AtomGrid::int_sides[1]; j++){
